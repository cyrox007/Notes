...
class Messenger {
	constructor() {
		...
		this.data = {
			...
			//dialogLinks: document.querySelectorAll('.messenger__dialog-list__item'), // Все блоки-кнопки контактов
			...
			dialogWindowContent: document.getElementById('messager-viewer'),
			dialogWindowEmpty: document.getElementById('messager-disable'),
	...
	loadDialog() {
		...
		let username = this.currentDialog.querySelector('#userfullname').textContent;
		...
		this.data.dialogWindowContent.querySelector('#userfullname').innerHTML = `<a href="/users/${user_uid}">${username}</a>`;
	...
	loadMessages() {
		...
		this.data.messages.forEach(msg => {
			...
			<div class="messenger__dialog-window__message" id="${msg.uid}" data-message_uid="${msg.uid}" data-user_uid="${msg.users.uid}" data-affiliation="${msg.users.uid == user_uid ? 'my' : 'me'}" data-status="${msg.message_status}">
				<div class="messenger__dialog-window__message_wrapper">
					<div class="messenger__dialog-window__message--img">
						<img src="${msg.users.user_image != 'default_img' ? msg.users.user_image : '/assets/img/default_avatar.png'}" alt="${msg.users.firstname} ${msg.users.surname} - ${msg.created_at}">
					</div>
					<div class="messenger__dialog-window__message--data">
						<div class="messenger__dialog-window__message--data_header">
							<span><b>${msg.users.firstname} ${msg.users.surname}</b></span> 
			...
			dialogMessages.innerHTML += messageBoxContent;
		});

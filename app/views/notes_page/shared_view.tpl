...
<section class="shared-note">
    ...
    {if $attachments}
        <div class="note-attachments-display">
            ...
            {foreach $attachments as $attachment}
                {assign var=attachmentUrl value={route_path name="note_shared_attachment" token=$shareToken fileUid=$attachment.file_uid}}
                <div class="attachment-item">
                    <div class="attachment-preview">
                        {if $attachment.file_type == 'voice' || $attachment.file_type == 'audio'}
                            <span class="attachment-icon">🎤</span>
                            <audio controls preload="metadata">
                                <source src="{$attachmentUrl|escape}" type="{$attachment.mime_type|escape}">
                                Ваш браузер не поддерживает аудио.
                            </audio>
                        {elseif $attachment.file_type == 'image'}
                            <img src="{$attachmentUrl|escape}" alt="{$attachment.file_name|escape}" loading="lazy" style="max-width:300px;max-height:240px;object-fit:contain;">
                        ...
                        {else}
                            <span class="attachment-icon">📄</span>
                            <a href="{$attachmentUrl|escape}">{$attachment.file_name|escape}</a>
                        {/if}
                    </div>
    ...
    <div class="share-actions">
        <a href="{route_path name='main'}" class="btn btn-secondary">← Вернуться на главную</a>
    </div>
</section>

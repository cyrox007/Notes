...
final class TwoFactorService
{
    ...
    public function provisioningUri(string $account, string $issuer, string $secret): string
    {
        ...
        $issuer = trim($issuer);
        if ($account === '' || $issuer === '') {
            throw new RuntimeException('Для TOTP обязательны имя учётной записи и издатель');
        }
        if (!self::isBase32Secret($secret)) {
            throw new RuntimeException('Некорректный секрет TOTP');
        }
    ...
    public function matchingCounter(
        ...
        // Сначала проверяем текущий интервал, затем допускаем отклонение на один
        // интервал в обе стороны. Уже использованный счётчик повторно не принимается.
        $offsets = [0, -1, 1];
    ...
    public function hashRecoveryCodes(array $codes): string
    {
        ...
        foreach ($codes as $code) {
            ...
            if (preg_match('/^[A-F0-9]{' . self::RECOVERY_RAW_LENGTH . '}$/D', $normalized) !== 1) {
                throw new RuntimeException('Некорректный формат резервного кода');
            }
    ...
    /**
     * Проверяет и атомарно поглощает либо TOTP-код, либо резервный код.
     *
    ...
    public function consumeRecoveryCodeHashSet(?string $json, string $code): ?string
    {
        ...
        try {
            ...
            throw new RuntimeException('Сохранённый набор резервных кодов некорректен', 0, $e);
        }
        if (!is_array($hashes) || !array_is_list($hashes)) {
            throw new RuntimeException('Сохранённый набор резервных кодов некорректен');
        }

        $candidate = hash('sha256', $normalized);
        foreach ($hashes as $index => $hash) {
            if (!is_string($hash) || preg_match('/^[a-f0-9]{64}$/D', $hash) !== 1) {
                throw new RuntimeException('Сохранённый хеш резервного кода некорректен');
            }

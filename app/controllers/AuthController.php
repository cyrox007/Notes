...
{
    /**
     * Политика регистрации, включая совместимость с устаревшим
     * REGISTRATION_INVITE_CODE, принадлежит RegistrationPolicyService.
     * Обработка секретов окружения вынесена из HTTP-контроллера, чтобы
     * административная политика оставалась единственным источником истины.
     */
    ...
    private function pendingRequiredTwoFactorEnrollment(Request $request): ?array
    {
        ...
        try {
            ...
            error_log('Не удалось расшифровать состояние обязательной настройки 2FA: ' . $e->getMessage());
            return null;
        }
    }

...
final class LicenseRecoveryController extends Controller
{
    public function index(Request $request): void
    {
        ...
        $flash = $request->session('system_license_flash');
        $request->unsetSession('system_license_flash');

        $this->render_template('core/license_recovery', [
            'user' => $user,
    ...
    {
        $request->setSession('system_license_flash', [
            'type' => $success ? 'success' : 'error',
            'message' => $message,
        ]);
        Router::getInstance()->redirect('system_license');
    }
}

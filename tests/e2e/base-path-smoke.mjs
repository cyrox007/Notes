...
try {
  ...
  if (uploadResponse.status() !== 200) {
    let uploadBody = '<response body unavailable>';
    try {
      uploadBody = await uploadResponse.text();
    } catch (_) {
      // A browser navigation can evict the response body. Status plus network
      // diagnostics still identify the failed upload contract.
    }
    throw new Error(

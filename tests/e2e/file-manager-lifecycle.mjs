...
try {
  ...
  // Create a folder through the actual modal and AJAX path.
  ...
  if (!folderHref || !folderHref.startsWith(`${basePath}/files/folder/`)) {
    throw new Error(`Folder URL escaped BASE_PATH: ${folderHref}`);
  }
  await Promise.all([
  ...
  // Upload a small text file. Fixture leaves 4096 bytes free, so this succeeds.
  ...
  // Preview the real protected file through the read-only text preview UI.
  await fileItem.dblclick();
  await page.locator('#text-preview-modal').waitFor({ state: 'visible', timeout: 5000 });
  await page.locator('#text-preview-content').filter({ hasText: `File Manager browser lifecycle ${stamp}` })
    .waitFor({ state: 'visible', timeout: 10000 });
  await page.locator('#text-preview-modal .file-manager__modal-close').click();

  // Download through the authenticated browser context and verify bytes + headers.
  let downloadHref = await fileItem.locator('a[title="Открыть"]').getAttribute('href');
  if (!downloadHref || !downloadHref.startsWith(`${basePath}/files/get/`)) {
    throw new Error(`Protected file URL escaped BASE_PATH: ${downloadHref}`);
  }
  let download = await context.request.get(origin + downloadHref);
  if (download.status() !== 200 || (await download.text()) !== fileContent) {
    throw new Error(`Authenticated download failed: HTTP ${download.status()}`);
  }
  if (!String(download.headers()['content-disposition'] || '').includes(fileName)) {
    throw new Error(`Unexpected download filename: ${download.headers()['content-disposition']}`);
  }

  // Rename through the real modal and verify the download header follows metadata.
  ...
  await waitForNavigation(page, () => page.locator('#modal-rename .modal-ok').click());

  fileItem = page.locator(`.file-manager__item[data-id="${fileId}"]`);
  await fileItem.locator('.file-manager__item-name').filter({ hasText: `${renamedStem}.txt` })
    .waitFor({ state: 'visible', timeout: 10000 });
  ...
  if (download.status() !== 200 || (await download.text()) !== fileContent) {
    throw new Error(`Renamed file download failed: HTTP ${download.status()}`);
  }
  if (!String(download.headers()['content-disposition'] || '').includes(`${renamedStem}.txt`)) {
    throw new Error(`Renamed download header is stale: ${download.headers()['content-disposition']}`);
  }

  // A 4096-byte upload must exceed the remaining quota and surface the server message to the user.
  quotaFailureExpected = true;
  const quotaDialogPromise = acceptDialog(page, 'Недостаточно места в персональном хранилище');
  await page.locator('#file-input').setInputFiles({
    name: overflowName,
    mimeType: 'text/plain',
    buffer: Buffer.alloc(4096, 66),
  });
  await quotaDialogPromise;
  quotaFailureExpected = false;
  if (await page.locator('.file-manager__item').filter({ hasText: overflowName }).count()) {
    throw new Error('Quota-rejected file appeared in File Manager');
  }

  // Delete the uploaded file. The folder becomes empty, so the UI reloads it.
  fileItem = page.locator(`.file-manager__item[data-id="${fileId}"]`);
  const deleteFileDialog = acceptDialog(page, `Удалить «${renamedStem}»?`);
  ...
  if (await page.locator(`.file-manager__item[data-id="${fileId}"]`).count()) {
    throw new Error('Deleted file is still visible');
  }

  // Return to root and delete the lifecycle folder through the real confirmation path.
  const rootHref = await page.locator('.file-manager__breadcrumb-item').first().getAttribute('href');
  if (!rootHref || rootHref.replace(/\/+$/, '') !== `${basePath}/files`) {
    throw new Error(`Root breadcrumb escaped BASE_PATH: ${rootHref}`);
  }
  await page.goto(origin + rootHref, { waitUntil: 'domcontentloaded' });
  folderItem = page.locator(`.file-manager__item[data-id="${folderId}"]`);
  await folderItem.waitFor({ state: 'visible', timeout: 10000 });
  const deleteFolderDialog = acceptDialog(page, `Удалить «${folderName}»?`);

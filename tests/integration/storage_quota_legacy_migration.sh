...
checkpoint() { echo "[legacy-storage] $*"; }
create_database() {
  local db="$1"
...
DBHOST="$DB_HOST" DBPORT="$DB_PORT" DBUSER="$DB_USER" DBPASS="$DB_PASS" DBNAME="$POSITIVE_DB" php bin/migrate.php --status | grep -Fq 'Schema contract: OK'

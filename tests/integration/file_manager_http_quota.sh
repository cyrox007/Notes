...
UNIQUE_KEY=file-http-unique-key-0123456789abcdef0123456789abcdef
MSG_SECRET_KEY=file-http-message-key-0123456789abcdef0123456789abcdef
WS_TICKET_SECRET=file-http-ticket-key-0123456789abcdef0123456789abcdef
EOF
chmod 600 .env
...
# Authentication is enforced by the real router middleware chain.
...
# Authenticated uploads still require a real CSRF token. Quota middleware runs
# before controller construction, so this also exercises shutdown lock release.
...
# Normal multipart upload persists metadata and the physical file.
...
# More than the remaining quota is rejected before FileController moves a file.
...
# Exactly the remaining bytes are allowed.
...
# Two independent PHP sessions for the same user avoid session-file serialization.
# With four PHP workers the requests overlap; the shared MySQL advisory lock must
# allow exactly one 4-byte upload against a 6-byte quota.
...
# Force metadata persistence to fail after move_uploaded_file(). The HTTP request
# must fail closed and FileController must remove the just-created physical file.

...
{
    fwrite(STDERR, "[FAIL] production trust domains: {$message}\n");
    exit(1);
}
...
function trustDecodePublic(string $encoded, string $domain, string $keyId): string
{
    if ($encoded === '' || preg_match('/^[A-Za-z0-9_-]+$/D', $encoded) !== 1) {
        trustFail("{$domain} public key {$keyId} is not canonical base64url");
    }
    ...
    if (!is_string($raw) || strlen($raw) !== SODIUM_CRYPTO_SIGN_PUBLICKEYBYTES) {
        trustFail("{$domain} public key {$keyId} is not a raw Ed25519 public key");
    }
    return $raw;
}

trustAssert(extension_loaded('sodium'), 'sodium extension is required');

$license = require $root . '/config/license_trusted_keys.php';
$update = require $root . '/config/update_trusted_keys.php';
trustAssert(is_array($license), 'license trust registry must return an array');
trustAssert(is_array($update), 'update trust registry must return an array');

$licenseFingerprints = [];
foreach ($license as $keyId => $encoded) {
    trustAssert(
        is_string($keyId) && preg_match('/^[A-Za-z0-9][A-Za-z0-9._-]{0,31}$/D', $keyId) === 1,
        'invalid license key id'
    );
    trustAssert(is_string($encoded), "license public key {$keyId} must be a string");
...
foreach ($update as $keyId => $encoded) {
    trustAssert(
        ...
        'invalid update key id'
    );
    trustAssert(is_string($encoded), "update public key {$keyId} must be a string");
...
trustAssert(
    ...
    'license and update key ids must be independent'
);
trustAssert(
    array_intersect(array_values($licenseFingerprints), array_values($updateFingerprints)) === [],
    'license and update signing domains must not reuse the same Ed25519 keypair'
);
...
trustAssert(
    ...
    'license private-key file domain prefix drifted'
);
trustAssert(
    str_contains($updateLib, "WORKSPACE_UPDATE_SECRET_PREFIX = 'wo-update-ed25519-secret-v1:'"),
    'update private-key file domain prefix drifted'
);
trustAssert(
    !str_contains($licenseLib, "wo-update-ed25519-secret-v1:"),
    'license tooling recognizes the update private-key file domain'
);
trustAssert(
    !str_contains($updateLib, "wo-ed25519-secret-v1:"),
    'update tooling recognizes the license private-key file domain'
);
...
trustAssert(
    ...
    'master release gate does not require production license public keys'
);
trustAssert(
    str_contains($releaseGate, 'Production update public key registry is empty'),
    'master release gate does not require production update public keys'
);
trustAssert(
    str_contains($releaseGate, 'License/update public keys must use independent key material'),
    'master release gate does not enforce independent signing key material'
);
...
foreach ([
    ...
    trustAssert(str_contains($ceremony, $marker), "trust ceremony runbook is missing marker: {$marker}");
}

fwrite(STDOUT, "[OK] production license and update trust domains remain independent\n");

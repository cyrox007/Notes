        ...
        if ($errors === []) {
            try {
                ...
                $_SESSION['notes_install_db'] = [
                    ...
                    'ws_public_url' => $wsPublicUrl, 'unique_key' => randomSecret(), 'message_key' => randomSecret(),
...
<main class="card">
    ...
    <?php if ($step === 1): ?>
        ...
        <div class="summary"><strong>Автоопределение</strong><p>Сайт: <code><?= htmlspecialchars(appUrl($detectedSiteUrl,$detectedBasePath), ENT_QUOTES, 'UTF-8') ?></code><br>WebSocket: <code><?= htmlspecialchars($detectedWsUrl, ENT_QUOTES, 'UTF-8') ?></code><br>Private storage: <code><?= htmlspecialchars($detectedPrivateStorage !== '' ? $detectedPrivateStorage : 'нужно указать', ENT_QUOTES, 'UTF-8') ?></code></p></div>
    ...
    <?php else: ?>
        ...
        <?php if ($hasMessenger): ?><p>Realtime Messenger использует встроенный native WebSocket process. Запустите <code>php ws_server/server.php start</code> через systemd/Supervisor/панель и проксируйте публичный <code>/ws</code> на локальный <code>WS_PORT</code>.</p><?php endif; ?>

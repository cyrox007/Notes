...
- Добавлена фоновая проверка подписанного stable-канала после входа и каждые пять минут во время активной работы интерфейса.
- В общей шапке появился центр системных уведомлений: при доступном совместимом релизе суперадминистратор видит новую версию и устанавливает её одной кнопкой.
- Одношаговая установка сама повторно проверяет signed feed, фиксирует exact `version_code` и SHA-256 пакета и передаёт их существующему транзакционному `bin/update_run.php`; предварительный ручной «Проверить обновления» больше не является обязательным.
- Транзакционный updater по-прежнему выполняет staging, backup кода и MySQL, candidate, maintenance, migrations, post-update healthcheck и автоматический rollback при штатной ошибке после начала изменения live-файлов.

### Совместимость и сервер обновлений
- Клиент передаёт control plane фактические `X-Notes-Version` и `X-Notes-Version-Code`; эти поля не дают дополнительных полномочий и используются только для выбора совместимого подписанного релиза.
- Новые установки считаются поддерживаемыми только при наличии `openssl`, `zlib`, `proc_open` и обнаруживаемого PHP CLI, чтобы невозможность будущего автообновления выявлялась во время установки.
- Переход `1.0.4 → 1.0.5` является последним переходным шагом через существующий раздел Admin → Updates. Начиная с `1.0.5 → 1.0.6` релизный контракт требует автоматического уведомления и одной кнопки без PowerShell, ручных миграций, bootstrap-файлов и редактирования `.env`.
- Stable release gate теперь отдельно проверяет наличие одношагового контура и его попадание в hosting bundle.

## 1.0.4 — 2026-09-24

### Admin → Updates
- Уплотнён раздел обновлений: статусные карточки стали компактнее, предупреждения больше не прилипают к границам карточки, а диагностика и ручная установка собраны в двухколоночный служебный блок.
- Инструкция recovery вынесена в отдельную спокойную подсказку, а список границ безопасности на широком экране использует две колонки без лишней вертикальной высоты.
- На узких экранах служебные команды и правила безопасности автоматически возвращаются в одну колонку.

### Windows / OSPanel
- Исправлен поиск PHP CLI из web-SAPI: кроме `UPDATE_PHP_BINARY` и `PHP_BINDIR` проверяется `php.exe` рядом с фактическим `PHP_BINARY`, а затем абсолютные каталоги из `PATH`.
- Это закрывает сценарий Open Server/OSPanel, где web-PHP работает из корректного каталога, но compile-time `PHP_BINDIR` указывает в другое место и Admin ошибочно сообщает, что CLI-бинарник не найден.
- Поиск не использует shell и принимает только абсолютные существующие пути; на POSIX дополнительно сохраняется проверка executable bit.

### Проверка обновления
- Сквозной update E2E переведён на предыдущую опубликованную версию `v1.0.3`: сначала exact `1.0.3` устанавливает текущий `1.0.4` штатным подписанным updater, затем обновлённая установка проверяет следующий синтетический пакет через Admin UI.
- Для первого шага используется версия и схема предыдущего опубликованного релиза, чтобы текущая canonical schema не маскировала несовместимость upgrade-path.
- `1.0.4` не добавляет новых таблиц или миграций и остаётся patch-релизом линии 1.0.

## 1.0.3 — 2026-09-24

### Обновление из админ-панели
- Admin → Updates теперь умеет не только проверять и подготавливать пакет, но и запускать реальную установку подписанного обновления через существующий транзакционный `bin/update_run.php`.
- Установка из браузера доступна только суперадминистратору, защищена CSRF и привязана к проверенным перед подтверждением `version_code` и SHA-256 пакета; если подписанный канал изменился между проверкой и установкой, операция прекращается до изменения рабочих файлов.
- Web-runtime отдельно находит PHP CLI, включая Windows/OSPanel; при отсутствии пригодного CLI кнопочная установка закрывается безопасно и оставляет оператору CLI-путь.
- Добавлен настоящий сквозной E2E реального пути обновления: отдельная exact-установка `v1.0.2` сначала выполняет единственный поддерживаемый CLI-переход на текущий `1.0.3`, затем уже работающая `1.0.3` через Admin → Updates устанавливает следующее подписанное тестовое обновление. Проверяются обе committed-транзакции, снятый maintenance, сохранность установки и post-update healthcheck.
- Исправлено определение остановленного WebSocket после русификации CLI: машинный код возврата `1` больше не зависит от английского текста `not running`.
- Если apply завершается ошибкой до `live_mutation_started`, updater снимает принадлежащий транзакции maintenance; после начала изменения live-файлов сохраняется fail-closed recovery boundary.

### Надёжность Admin и схемы
- Страница управления ролями больше не превращает ошибку чтения данных в необработанный HTTP 500: диагностическое состояние возвращается контролируемо.
- До обычного HTTP-runtime добавлена проверка фактической готовности схемы; несовместимая или не полностью обновлённая БД получает диагностический HTTP 503 «Требуется обновление базы данных» вместо случайных SQL 500 на отдельных маршрутах.
- Проверка `information_schema` больше не зависит от регистра alias/ключей результата конкретного MySQL-драйвера.

### Интерфейс
- Стабилизированы состояния регистрации в Admin и Messenger при переключении WebSocket ↔ HTTP long-poll fallback.
- Сохранена единая логика realtime без резких визуальных скачков при временной потере WebSocket.

### Переход с 1.0.2
- Опубликованная `1.0.2` ещё не умеет выполнять destructive apply из Admin UI, а её CLI updater содержит дефект распознавания локализованного статуса остановленного WebSocket. Для перехода `1.0.2 → 1.0.3` выпускается отдельный `bootstrap-1.0.2-updater.php`: он работает только на exact `1.0.2 (10002)`, атомарно исправляет известную проверку exit code и запускает штатный подписанный updater. Bootstrap и его SHA-256 входят в immutable release-candidate artifact.
- Начиная с `1.0.3`, последующие подписанные обновления можно устанавливать кнопкой из Admin → Updates при выполненной readiness-проверке.

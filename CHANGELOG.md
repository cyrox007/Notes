...
### Переход с 1.0.2
- Опубликованная `1.0.2` ещё не умеет выполнять destructive apply из Admin UI. Поэтому переход `1.0.2 → 1.0.3` выполняется один раз через `php bin/update_run.php --yes --json` после публикации подписанного stable feed.

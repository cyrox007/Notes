...
## Отдельная security-задача после 1.0.2 — двухфакторная аутентификация TOTP

После завершения стабилизации `1.0.2` требуется отдельная задача по двухфакторной аутентификации. Она не входит в updater scope `1.0.2` и не должна смешиваться с релизной стабилизацией.

Целевой контракт:

- стандарт TOTP (RFC 6238), совместимый с Google Authenticator, Microsoft Authenticator, Aegis, 2FAS и другими приложениями без привязки к конкретному поставщику;
- подключение через QR-код и ручной Base32 secret;
- первый корректный TOTP-код обязателен до фактического включения 2FA;
- login разделяется на password step и отдельный TOTP challenge; до успешного challenge полноценная authenticated session не создаётся;
- одноразовые recovery codes с хранением только их хэшей;
- включение/отключение и recovery защищены повторной проверкой пароля и журналируются;
- rate limiting для TOTP/recovery attempts;
- TOTP secret хранится только в зашифрованном виде с отдельным crypto context/AAD;
- admin reset 2FA — отдельная привилегированная, журналируемая операция;
- browser/E2E и security regression coverage для enrollment, login, recovery, disable/reset.

Жёсткое архитектурное требование: реализация остаётся **vendor-free**. Нельзя добавлять Composer/npm/CDN/runtime зависимости, Google API или внешний QR/TOTP service. TOTP/Base32/provisioning и QR generation должны быть встроенными primitives проекта; QR предпочтительно генерировать локально как SVG.

Несколько методов MFA, SMS/email-коды и обязательная 2FA по ролям/организации могут рассматриваться отдельными следующими этапами после базового TOTP contract.

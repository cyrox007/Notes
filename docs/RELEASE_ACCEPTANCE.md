...
- обязательный `admin-update-e2e`: exact опубликованная `v1.0.4` устанавливает текущий `1.0.5` через штатный Admin → Updates; затем корректно подписанный тестовый `1.0.6` с намеренно падающей миграцией устанавливается из автоматического уведомления и обязан самостоятельно вернуть `1.0.5` в `rollback_verified` без maintenance; после переключения feed на исправный `1.0.6` та же установка снова обнаруживает обновление автоматически и завершает его одной кнопкой в `committed`; отдельный этап имитирует аварийно оставшийся валидный maintenance-marker и требует, чтобы следующий HTTP-запрос завершил recovery до запуска БД/модулей;
...
6. На exact `1.0.4` в Windows/OSPanel 5.2.2 выполните последний переходный сценарий Admin → Updates → Проверить → Установить и подтвердите обновление до `1.0.5` без ручного `UPDATE_PHP_BINARY`, PowerShell, bootstrap или миграций. После запуска `1.0.5` сначала дайте корректно подписанный тестовый `1.0.6` с намеренным отказом миграции и подтвердите автоматический возврат на `1.0.5` без CLI-команд и оставшегося maintenance. Затем переключите signed feed на исправный `1.0.6`, вернитесь в обычный интерфейс и подтвердите: уведомление появляется автоматически, а единственное нажатие «Обновить до 1.0.6» завершает транзакцию `committed`. Отдельно имитируйте аварийно оставшийся валидный maintenance-marker и подтвердите, что первый следующий HTTP-запрос сам завершает recovery до инициализации БД.

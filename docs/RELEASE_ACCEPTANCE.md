# Финальная приёмка релиза Workspace Organizer 1.0.5

Этот документ — финальный чек-лист выпуска `v1.0.5`. Он намеренно разделяет корректность репозитория, доказательства CI и действия, которые может выполнить только оператор. Релиз принимается только для одного точного commit; доказательства со старого HEAD автоматически не переносятся.
...
5. README, CHANGELOG и `docs/releases/v1.0.5.md` описывают одну и ту же версию и статус.
...
- обязательный `admin-update-e2e`: exact опубликованная `v1.0.4` устанавливает текущий `1.0.5` через штатный Admin → Updates; затем корректно подписанный тестовый `1.0.6` с намеренно падающей миграцией устанавливается из автоматического уведомления и обязан самостоятельно вернуть `1.0.5` в `rollback_verified` без maintenance; второй подписанный `1.0.6` обязан пройти миграции, намеренно провалить post-health и снова автоматически вернуть `1.0.5` с новым `rollback_verified`; после переключения feed на исправный `1.0.6` та же установка снова обнаруживает обновление автоматически и завершает его одной кнопкой в `committed`; отдельный этап имитирует аварийно оставшийся валидный maintenance-marker и требует, чтобы следующий HTTP-запрос завершил recovery до запуска БД/модулей;
...
6. На exact `1.0.4` в Windows/OSPanel 5.2.2 выполните последний переходный сценарий Admin → Updates → Проверить → Установить и подтвердите обновление до `1.0.5` без ручного `UPDATE_PHP_BINARY`, PowerShell, bootstrap или миграций. После запуска `1.0.5` сначала дайте корректно подписанный тестовый `1.0.6` с намеренным отказом миграции и подтвердите автоматический возврат на `1.0.5` без CLI-команд и оставшегося maintenance. Затем повторите тест с подписанным `1.0.6`, у которого миграции проходят, но post-health намеренно завершается ошибкой, и снова подтвердите автоматический возврат на `1.0.5`. Только после двух подтверждённых откатов переключите signed feed на исправный `1.0.6`, вернитесь в обычный интерфейс и подтвердите: уведомление появляется автоматически, а единственное нажатие «Обновить до 1.0.6» завершает транзакцию `committed`. Отдельно имитируйте аварийно оставшийся валидный maintenance-marker и подтвердите, что первый следующий HTTP-запрос сам завершает recovery до инициализации БД.
...
- exact frozen RC прошёл одношаговый updater acceptance: после перехода на `1.0.5` тестовый следующий signed-релиз обнаруживается автоматически в обычном интерфейсе и устанавливается одной кнопкой без ручной проверки канала;
...
4. Создайте подписанный/аннотированный тег `v1.0.5` согласно release policy репозитория.

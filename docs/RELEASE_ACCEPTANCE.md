# Финальная приёмка релиза Workspace Organizer 1.0.4

Этот документ — финальный чек-лист выпуска `v1.0.4`. Он намеренно разделяет корректность репозитория, доказательства CI и действия, которые может выполнить только оператор. Релиз принимается только для одного точного commit; доказательства со старого HEAD автоматически не переносятся.
...
5. README, CHANGELOG и `docs/releases/v1.0.4.md` описывают одну и ту же версию и статус.
...
- обязательный `admin-update-e2e`: exact-установка опубликованного `v1.0.3` штатно обновляется до текущего `1.0.4`, затем уже `1.0.4` устанавливает следующее подписанное тестовое обновление из Admin UI; проверяются обе committed-транзакции, снятый maintenance и post-update healthcheck;
...
6. На exact `1.0.3` откройте Admin → Updates, выполните проверку подписанного канала и подтвердите, что `1.0.4` устанавливается только при готовом updater runtime. На Windows/OSPanel отдельно подтвердите автоматическое обнаружение `php.exe` без ручного `UPDATE_PHP_BINARY`, если CLI находится рядом с web-PHP или в абсолютном каталоге `PATH`. Для опубликованной `1.0.2` сохраняется последовательный путь через `bootstrap-1.0.2-updater.php` до `1.0.3`, а уже затем обычное обновление до `1.0.4`.
...
4. Создайте подписанный/аннотированный тег `v1.0.4` согласно release policy репозитория.

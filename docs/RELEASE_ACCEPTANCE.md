...
6. На exact `1.0.4` в Windows/OSPanel 5.2.2 выполните последний переходный сценарий Admin → Updates → Проверить → Установить и подтвердите обновление до `1.0.5` без ручного `UPDATE_PHP_BINARY`, PowerShell, bootstrap или миграций. После запуска `1.0.5` сначала дайте корректно подписанный тестовый `1.0.6` с намеренным отказом миграции и подтвердите автоматический возврат на `1.0.5` без CLI-команд и оставшегося maintenance. Затем переключите signed feed на исправный `1.0.6`, вернитесь в обычный интерфейс и подтвердите: уведомление появляется автоматически, а единственное нажатие «Обновить до 1.0.6» завершает транзакцию `committed`.

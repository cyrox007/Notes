...
- обязательный `admin-update-e2e`: exact опубликованная `v1.0.4` устанавливает текущий `1.0.5` через штатный Admin → Updates, после чего уже работающая `1.0.5` обнаруживает тестовый `1.0.6` автоматически в центре уведомлений и устанавливает его одной кнопкой; проверяются обе committed-транзакции, снятый maintenance и post-update healthcheck;
...
6. На exact `1.0.4` в Windows/OSPanel 5.2.2 выполните последний переходный сценарий Admin → Updates → Проверить → Установить и подтвердите обновление до `1.0.5` без ручного `UPDATE_PHP_BINARY`, PowerShell, bootstrap или миграций. После запуска `1.0.5` переключите тестовый signed feed на совместимый `1.0.6`, вернитесь в обычный интерфейс и подтвердите: уведомление появляется автоматически, а единственное нажатие «Обновить до 1.0.6» завершает транзакцию `committed`.

...
| R04 CI convergence | Corrective convergence prepared | PR #164 established the final baseline; the final source-convergence patch additionally removes stale pre-isolation workflow paths across Messenger/Notes/Profile/Files, refreshes compatibility fixtures, binds packaging to version/source SHA and hardens the customer bundle surface | Merge the final source-convergence PR, then run one final agreed release run on the frozen SHA | Pending merge of final source convergence |
| R05 GitHub merge governance | Corrective source patch prepared | Release governance contract and protection applicator exist; live application exposed a literal `\\n` status-context serialization bug, fixed by the final source-convergence patch with fail-closed post-apply verification | Merge the corrective source patch, then owner/admin must apply and verify actual GitHub protection on `1.0` and `master` | Source patch pending merge; operator pending |
